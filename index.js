...
const pizza = createMenuItem("pizza", 4, "Lunch\\Dinner");
console.log(pizza);
...
console.log(reviews.filter(rev => rev.name === "Julius")[0].feedback);
...
const newReview = { name: "Kelly", rating: 2, feedback: "I've never been there, but they deserve a 2." };
...
reviews.filter(rev => rev.name === "Reyna")[0].feedback = "this place is chill with really cool people, great for getting work done on weekdays";
console.log(reviews);
...
function getLastReview(array, index) {
  const rev = array[array.length - 1];
  return `${rev.name} gave the restaurant a ${rev.rating} star review, and their feedback was: ${rev.feedback}`;
}
